...
const transformBookingData = (data: any[]): ScheduleItem[] =>
  data.map((item) => {
    const startDate = item.time_in
      ? dayjs.utc(item.time_in).tz("Asia/Manila", true).toDate()
      : null;
    const endDate = item.time_out
      ? dayjs.utc(item.time_out).tz("Asia/Manila", true).toDate()
      : null;
...
const transformScheduleData = (data: any[]): ScheduleItem[] => {
  return data.flatMap((item) => {
    ...
    try {
      ...
      console.error(
        `Error processing days for item: ${JSON.stringify(item)}`,
        error
      );
      return [];
    }
...
const SchedulePage: React.FC = () => {
  ...
  useEffect(() => {
    if (bookedRoomsError || scheduleDataError || roomsError) {
      console.error(
        "Error fetching data:",
        bookedRoomsError || scheduleDataError || roomsError
      );
      setIsLoading(false);
      return;
    }
    ...
    if (bookedRoomsData && scheduleDataFromAPI && roomsData) {
      ...
      const validData = [
        ...
        (item) =>
          item.status && validStatuses.includes(item.status.toUpperCase())
      );
